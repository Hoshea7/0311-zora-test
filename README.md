...
所有敏感凭证通过 Electron `safeStorage` 加密保存，避免明文落盘。
...
| `safeStorage` | 使用系统能力加密 API Key 与敏感配置 |
...
│   │   ├── provider-manager.ts     # Provider 管理与加密存储
...
```text
~/.zora/
├── providers.json                # Provider 配置（密钥加密保存）
├── feishu-config.json            # 飞书配置（密钥加密保存）
