...
Zora 搭配顶级Agent能力，帮助你达成你的办公任务。你可以设置工作区，zora将在当前工作区处理代码、调研、写作和项目资料；在飞书中，它可以通过私聊或群聊接收任务，并在对应工作区继续执行（出差、旅游的时候就可以通过手机操作）。关于记忆、项目背景、重要决定、每日记录、技能方法和工具入口等会以本地文件或配置形式保存；文件写入、命令执行等操作由权限模式控制。
